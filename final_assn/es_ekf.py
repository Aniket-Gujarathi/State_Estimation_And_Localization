# @Author: aniket
# @Date:   2019-11-03T12:46:49+05:30
# @Last modified by:   aniket
# @Last modified time: 2019-11-08T19:56:54+05:30
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    I = np.eye(3)
    R = I * sensor_var
    K_k = p_cov_check.dot(h_jac.T).dot(linalg.inv(h_jac.dot(p_cov_check).dot(h_jac.T) + R))
    # 3.2 Compute error state

    del_k = K_k.dot(y_k - p_check)
    ...
    q_hat = Quaternion(euler = del_phi).quat_mult_right(q_check)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    rotn = Quaternion(*q_est[k-1]to_mat)
    # 1.1 Linearize the motion model and compute Jacobians
    p_est[k] = p_est[k-1] + v_est[k-1] * delta_t + (delta_t**2/2)(rotn.dot(imu_f.data[k-1]) + g)
    v_est[k] = v_est[k-1] + rotn.dot(imu_f.data[k-1] + g) * delta_t
    q_est[k] = Quaternion()
